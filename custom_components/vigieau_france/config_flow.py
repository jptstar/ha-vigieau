...
class VigiEauFranceConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None = None):
        ...
        if user_input is not None:
            ...
            try:
                self._candidates = await api.async_search_addresses(str(user_input[CONF_ADDRESS]))
                ...
                if not self._candidates:
                    ...
                    if len(self._candidates) == 1:
                        return await self._async_location_selected(self._candidates[0])
                    return await self.async_step_location()

        schema = vol.Schema(
            {
                vol.Required(CONF_ADDRESS): str,
                vol.Required(CONF_WATER_TYPE, default=DEFAULT_WATER_TYPE): vol.In(WATER_CHOICES),
    ...
    async def async_step_location(self, user_input: dict[str, Any] | None = None):
        if user_input is not None:
            ...
            return await self._async_location_selected(self._candidates[index])
        choices = {str(index): candidate.label for index, candidate in enumerate(self._candidates)}
        ...
        try:
            ...
            return self.async_show_form(
                ...
                data_schema=vol.Schema(
                    {
                        vol.Required(CONF_ADDRESS): str,
                        vol.Required(CONF_WATER_TYPE, default=self._pending_water_type): vol.In(WATER_CHOICES),
                        vol.Required(CONF_PROFILE, default=self._pending_profile): vol.In(PROFILE_CHOICES),
                    }
                ),
...
class VigiEauFranceOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input: dict[str, Any] | None = None):
        if user_input is not None:
            ...
            matching = zones_for_water_type(self._zones, str(self._pending[CONF_WATER_TYPE]))
            if len(matching) > 1:
                return await self.async_step_zone()
            if matching:
        ...
        current_water = self.config_entry.options.get(
            CONF_WATER_TYPE, self.config_entry.data.get(CONF_WATER_TYPE, DEFAULT_WATER_TYPE)
        )
        current_interval = self.config_entry.options.get(CONF_SCAN_INTERVAL, DEFAULT_SCAN_INTERVAL)
        schema = vol.Schema(
            {
                vol.Required(CONF_WATER_TYPE, default=current_water): vol.In(WATER_CHOICES),
                vol.Required(CONF_PROFILE, default=current_profile): vol.In(PROFILE_CHOICES),
